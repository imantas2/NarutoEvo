...
client/verb/NinUp()
	set hidden = 1
	if(!usr) return
	usr.Nin_QTE_ArrowPressed(NORTH)
client/verb/NinDown()
	set hidden = 1
	if(!usr) return
	usr.Nin_QTE_ArrowPressed(SOUTH)
client/verb/NinLeft()
	set hidden = 1
	if(!usr) return
	usr.Nin_QTE_ArrowPressed(EAST)
client/verb/NinRight()
	set hidden = 1
	if(!usr) return
	usr.Nin_QTE_ArrowPressed(WEST)
mob/var/tmp/
	ChakraJutsuState = 0
	ChakraJutsuIndex = 0
...
mob
	var/tmp/sheilded=0
	var/nin_qte_active = 0
	var/nin_qte_streak = 0
	var/nin_qte_dir = 0
	var/nin_qte_time = 0
	obj/screen/nin_arrow/nin_qte_arrow = null
	var/tmp/gates[5]
	var/tmp/guardians[1]
...
mob/proc/
	...
	Nin_Training()
		...
		if(src.nin_qte_active) return
			...
			if(loc.loc:Safe!=1) src.LevelStat("Ninjutsu",rand(4,8))
			src.Levelup()
			src.nin_qte_streak = 0
			src.nin_qte_active = 1
			src.nin_qte_dir = 0
			if(J.level<4)
				if(loc.loc:Safe!=1) J.exp+=rand(2,5)
				J.Levelup()
			var/list/DIRS=list(NORTH,SOUTH,EAST,WEST)
			src << "<span class='notice'><b>Nin Training!</b> Press matching arrows within 1.5s. Streak: [src.nin_qte_streak]</span>"
			src.verbs += list(/client/verb/NinUp, /client/verb/NinDown, /client/verb/NinLeft, /client/verb/NinRight)
			spawn() src.Nin_QTE_Loop()
			src.move=0
			src.injutsu=1
			src.canattack=0
			src.firing=1
			...
			while(nin_qte_streak < 50 && !Prisoned)  // Max 20 arrows
				if(!ChakraCheck(50))
					src << "<span class='warning'>No chakra! Training ends (streak: [nin_qte_streak])</span>"
					src.firing=0
					break
				...
				spawn(25)  // 1.7s limit
					while(src.ArrowTasked == A)
						nin_qte_streak++
						src << "<span class='boldnotice'>Success! Streak: [nin_qte_streak]</span>"
				...
				spawn(17)
					if(src.ArrowTasked -= A)  // "Press" = face arrow dir
						src << "<span class='warning'>Timeout! Streak: [nin_qte_streak]</span>"
						del(A)
						src.ArrowTasked = null
				sleep(20)
			src.copy = null
			...
			src.canattack=1
			src.firing=0
			src << "<span class='notice'>Training done! Ninjutsu gained: [nin_qte_streak]</span>"
			src.ArrowTasked=null
			Effects["Nin Training"]=null
			J.Excluded = 1
			J.uses++
			...
			spawn()
				...
				J.JutsuCoolDown(src)
	Nin_QTE_Loop()
		while(src.nin_qte_active)
			src.Nin_QTE_NewArrow()
			sleep(20)
	Nin_QTE_NewArrow()
		if(!src.nin_qte_active) return
		if(src.nin_qte_arrow)
			var/client/C1 = src.client
			if(C1)
				C1.screen -= src.nin_qte_arrow
			del(src.nin_qte_arrow)
			src.nin_qte_arrow = null
		var/arrow_dir = pick(NORTH, SOUTH, EAST, WEST)
		src.nin_qte_dir = arrow_dir
		src.nin_qte_time = world.time
		var/client/C2 = src.client
		if(C2)
			var/obj/screen/nin_arrow/A = new
			A.dir = arrow_dir
			A.screen_loc = "CENTER, NORTH-1"
			C2.screen += A
			src.nin_qte_arrow = A
		spawn(15) 
			src.Nin_QTE_CheckTimeout()
	Nin_QTE_DirText(arrow_dir)
		if(arrow_dir == NORTH)
			return "UP"
		if(arrow_dir == SOUTH)
			return "DOWN"
		...
		if(arrow_dir == WEST)
			return "LEFT"
		return "?"
	Nin_QTE_CheckTimeout()
		if(!src.nin_qte_active) return
		if(!src.nin_qte_dir) return
		if(world.time - src.nin_qte_time >= 15)
			src.Nin_QTE_End("You took too long! Try again. Streak: [src.nin_qte_streak]")
	Nin_QTE_ArrowPressed(arrow_dir)
		if(!src.nin_qte_active) return 0
		if(!src.nin_qte_dir) return 0
		if(arrow_dir != src.nin_qte_dir)
			src.Nin_QTE_End("Wrong arrow! Streak: [src.nin_qte_streak]")
			return 0
		if(world.time - src.nin_qte_time > 15)
			src.Nin_QTE_End("Too late! Streak: [src.nin_qte_streak]")
			return 0
		src.nin_qte_streak += 1
		src.nin_qte_dir = 0
		src << "Correct! Streak: [src.nin_qte_streak]"
		if(src.nin_qte_arrow)
			var/client/C = src.client
			if(C)
				C.screen -= src.nin_qte_arrow
			del(src.nin_qte_arrow)
			src.nin_qte_arrow = null
		return 1
	Nin_QTE_End(msg)
		if(!src.nin_qte_active) return
		src.nin_qte_active = 0
		if(src.nin_qte_arrow)
			var/client/C = src.client
			if(C)
				C.screen -= src.nin_qte_arrow
